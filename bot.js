...
function doMerge(ws, forced){
  ...
  // Window P&L for REPORTING includes rebates (they're part of the edge).
  // But rebates were already credited to state.balance at fill time, so
  // when updating state.totalPnl we only add the NEW cash flows from merge:
  //   mergeProceeds + takerProceeds - takerFees - upSpent - dnSpent
  // (rebates were already reflected in balance and should not be double-counted)
  const totalIn   = mergeProceeds + totalTakerProceeds - totalTakerFees
                    + ws.upRebates + ws.dnRebates;  // for display / reporting
  const totalOut  = ws.upSpent + ws.dnSpent;
  const windowPnl = fl4(totalIn - totalOut);  // full P&L incl rebates (display)

  // For state.totalPnl: rebates already added at fill time — only add new cash
  const pnlDelta = fl4(mergeProceeds + totalTakerProceeds - totalTakerFees - totalOut);
  state.totalPnl = fl4(state.totalPnl + pnlDelta);
...
async function checkResolution(){
  ...
  for (const pw of toResolve){
    ...
    // Rebates were credited to balance at fill time — DO NOT add again.
    // windowPnl for display includes rebates; pnlDelta for state.totalPnl does not.
    const totalRebates = fl4((pw.upRebates || 0) + (pw.dnRebates || 0));
    const windowPnl    = fl4(totalProceeds + totalRebates - totalSpent);  // display
    const pnlDelta     = fl4(totalProceeds - totalSpent);                 // state
    state.totalPnl     = fl4(state.totalPnl + pnlDelta);
